...
if "results_v5" in st.session_state:
    ...
    st.subheader("📉 Today’s Decliners")
    st.caption("Stocks that are down in the current session. This is separate from a true multi-week or multi-month pullback.")
    decliners = results[results["Day Change %"] < 0].copy()
    decliners = decliners.sort_values(["Day Change %", "Alpha Business Score"], ascending=[True, False]).head(15)
    if decliners.empty:
        st.info("No scanned stocks are currently down today.")
    else:
        st.dataframe(
            decliners[["Ticker","Price","Day Change %","Intraday Volume Pace","Alpha Business Score","Accumulation Score","TradingView"]],
            use_container_width=True, hide_index=True,
            column_config={
                "Price": st.column_config.NumberColumn(format="$%.2f"),
    ...
    st.subheader("🧲 Major Pullbacks 2.0")
    st.caption("Ranks quality companies trading meaningfully below their own 52-week highs—even when they are green today.")
    pullbacks = results.copy()

    # Pullback depth score: the sweet spot rewards real corrections without blindly
    # treating catastrophic declines as bargains.
    depth = pullbacks["From 52W High %"].abs().fillna(0)
    pullbacks["Pullback Depth Score"] = np.select(
        [depth.between(10, 15), depth.between(15, 25), depth.between(25, 40), depth > 40, depth.between(5, 10)],
        [65, 85, 75, 40, 35],
        default=0,
    ).astype(float)

    rsi_score = ((60 - pullbacks["RSI 14"].fillna(60)) / 30 * 100).clip(0, 100)
    trend_score = (
        np.where(pullbacks["200D MA Slope %"].fillna(-99) >= 0, 55, 15)
        + np.where(pullbacks["Price"] >= pullbacks["200D MA"], 25, 0)
        + np.where(pullbacks["50D MA Slope %"].fillna(-99) >= 0, 20, 0)
    )
    relative_strength_score = ((pullbacks["RS vs SPY 3M %"].fillna(-20) + 20) / 40 * 100).clip(0, 100)

    pullbacks["Major Pullback Score"] = (
        pullbacks["Alpha Business Score"].fillna(0) * 0.38
        + pullbacks["Pullback Depth Score"] * 0.24
        + pullbacks["Accumulation Score"].fillna(0) * 0.14
        + rsi_score * 0.08
        + trend_score * 0.10
        + relative_strength_score * 0.06
    ).clip(0, 100).round(1)

    def classify_pullback(row):
        dd = abs(row.get("From 52W High %", np.nan))
        slope = row.get("200D MA Slope %", np.nan)
        price = row.get("Price", np.nan)
        ma200 = row.get("200D MA", np.nan)
        business = row.get("Alpha Business Score", 0)
        accumulation = row.get("Accumulation Score", 0)
        if pd.isna(dd):
            return "Unknown"
        if dd > 40 and (business < 55 or (pd.notna(slope) and slope < -5)):
            return "⚫ Possible value trap"
        if pd.notna(price) and pd.notna(ma200) and price < ma200 and pd.notna(slope) and slope < 0:
            return "🔴 Broken trend"
        if 25 <= dd <= 40:
            return "🟠 Deep correction"
        if 15 <= dd < 25:
            return "🟡 Market correction"
        if 10 <= dd < 15 and business >= 60:
            return "🟢 Healthy pullback"
        if 5 <= dd < 10:
            return "Normal dip"
        return "Extended drawdown"

    def pullback_reason(row):
        bits = [f'{abs(row["From 52W High %"]):.1f}% below 52W high']
        if row.get("Alpha Business Score", 0) >= 75:
            bits.append("high-quality business")
        elif row.get("Alpha Business Score", 0) >= 60:
            bits.append("solid business quality")
        if row.get("Accumulation Score", 0) >= 65:
            bits.append("accumulation remains constructive")
        if pd.notna(row.get("RSI 14")) and row.get("RSI 14") <= 40:
            bits.append("RSI is washed out")
        if pd.notna(row.get("200D MA Slope %")) and row.get("200D MA Slope %") >= 0:
            bits.append("long-term trend still rising")
        return ", ".join(bits[:4])

    pullbacks["Pullback Type"] = pullbacks.apply(classify_pullback, axis=1)
    pullbacks["Pullback Why"] = pullbacks.apply(pullback_reason, axis=1)

    # A true pullback begins 10% below the 52-week high. Keep weaker businesses
    # visible only when the score supports further research.
    pullbacks = pullbacks[(pullbacks["From 52W High %"] <= -10) & (pullbacks["Alpha Business Score"].fillna(0) >= 45)]
    pullbacks = pullbacks.sort_values(["Major Pullback Score","Alpha Business Score"], ascending=False).head(25)

    if pullbacks.empty:
        st.info("No qualifying 10%+ pullbacks were found in the scanned universe.")
    else:
        p1, p2, p3 = st.columns(3)
        p1.metric("Quality pullbacks", int((pullbacks["Alpha Business Score"] >= 70).sum()))
        p2.metric("Corrections 15%+", int((pullbacks["From 52W High %"] <= -15).sum()))
        p3.metric("Top opportunity", pullbacks.iloc[0]["Ticker"])
        pb_cols = ["Ticker","Price","52W High","From 52W High %","Major Pullback Score","Pullback Type","RSI 14","Distance From 200D MA %","Alpha Business Score","Accumulation Score","Pullback Why","TradingView"]
        st.dataframe(
            pullbacks[pb_cols], use_container_width=True, hide_index=True,
            column_config={
                "Price": st.column_config.NumberColumn(format="$%.2f"),
                "52W High": st.column_config.NumberColumn(format="$%.2f"),
                "From 52W High %": st.column_config.NumberColumn(format="%.2f%%"),
                "Major Pullback Score": st.column_config.ProgressColumn(format="%.1f", min_value=0, max_value=100),
